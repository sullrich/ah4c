...
# scripts/<device>/<provider>/stopbmitune.sh.
...
targetScript="$scriptDir/../../$device${provider:+/$provider}/stopbmitune.sh"

#Trap end of script run
finish() {
  echo "stopbmitune.sh (all/all) is exiting for $streamerIP with exit code $?"
}
...
dispatch() {
  if [[ ! -x "$targetScript" ]]; then
    echo "No stopbmitune.sh found for device=$device provider=$provider ($targetScript)"
    exit 1
  fi

  echo "Dispatching stopbmitune.sh to $device/$provider for $streamerIP"
