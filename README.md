...
the selected package, and only when it is missing or `UPDATE_SCRIPTS=Yes`; it never downloads
...
`stopbmitune.sh`; additional helper files are allowed. `UPDATE_SCRIPTS=Yes` works when
...
Four of these — `GPU_DEVICE`, `DOCKER_RUNTIME`, `NVIDIA_VISIBLE_DEVICES` and
`NVIDIA_DRIVER_CAPABILITIES` — drive GPU access, used by the closed-caption engine and, on
NVIDIA, by any `CMDn` that calls `h264_nvenc`/`hevc_nvenc`. Leaving them at their defaults
(`GPU_DEVICE=/dev/null`, `DOCKER_RUNTIME=runc`, the two NVIDIA variables empty) does nothing:
captions run on the processor and no `CMDn` can reach the GPU. `GPU_DEVICE` needs to point at
something that exists, which is why the default is `/dev/null` rather than empty. To use a GPU:

| Variable | Default | For an Intel or AMD GPU (Vulkan) | For an NVIDIA GPU |
| --- | --- | --- | --- |
| `GPU_DEVICE` | `/dev/null` | `/dev/dri` | leave at the default |
| `DOCKER_RUNTIME` | `runc` | leave at the default | `nvidia` |
| `NVIDIA_VISIBLE_DEVICES` | empty | leave empty | `all` (or a specific `GPU-…` UUID) |
| `NVIDIA_DRIVER_CAPABILITIES` | empty | leave empty | `compute,utility` for captions; add `video` (so `compute,utility,video`, or just `all`) if a `CMDn` uses NVENC |
